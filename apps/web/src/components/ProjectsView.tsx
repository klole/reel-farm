...
export default function ProjectsView({ initialProjects, ownerName }: { initialProjects: Project[]; ownerName: string }) {
  ...
  const [worker, setWorker] = useState<"ready" | "offline" | "checking">("checking");
  useEffect(() => { void apiFetch<{ worker: "ready" | "offline" }>("/api/health/details").then((health) => setWorker(health.worker)).catch(() => setWorker("offline")); }, []);
