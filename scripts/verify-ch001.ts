...
const runId = process.env.CH001_RUN_ID ?? `r2-${generatedAt.toISOString().replace(/[-:.TZ]/g, "").slice(0, 14)}`;
...
async function sourceReview(): Promise<{ path: string; clean: boolean }> {
  ...
  if (!state.includes("awaiting_review") || !state.includes("Accepted application version: none")) violations.push("root state is not awaiting review with accepted version unset"); else checks.push("root state remains awaiting review and unaccepted");
  const reportPath = resolve(evidenceDir, "source-review.md");
  await writeFile(reportPath, ["# CH-001R-r2 source/provenance inspection", "", `Implementation commit: ${implementationCommit}`, `Run ID: ${runId}`, "Evidence type: E1; source-only inspection, not runtime proof.", "", ...checks.map((check) => `- PASS — ${check}`), ...violations.map((violation) => `- FAIL — ${violation}`), "", "Pending repository license decision remains recorded; this inspection makes no legal/distribution determination."].join("\n") + "\n", { mode: 0o640 });
...
const sourceRef = await makeEvidenceRef(root, source.path, "E1", "source-review:ch001r2", implementationCommit);
const gates: GateRecord[] = template.map((gate) => {
  const evidence = suiteEvidence.get(gate.id);
  if (evidence && evidence.status === "PASS" && evidence.implementation_commit === implementationCommit) {
    const actualEvidence = [...(evidence.actual_evidence ?? [])];
    if (source.clean && template.find((candidate) => candidate.id === gate.id)?.required_evidence.includes("E1") && !actualEvidence.some((ref) => ref.kind === "E1")) actualEvidence.push(sourceRef);
    return { ...evidence, actual_evidence: actualEvidence };
  }
...
const report: EvidencePackage = { chapter: "CH-001R-r2", target_application_version: "0.1.0", report_kind: "REPAIR_RUN_WITH_HARNESS_AND_ENVIRONMENT_LIMITATIONS", implementation_commit: implementationCommit, evidence_commit: null, generatedAt: new Date().toISOString(), commands, suites: await readSuiteReports(), gates, findings, artifact_manifest: artifactRef };
const reportPath = resolve(evidenceDir, "gate-results.json");
await writeJson(reportPath, report);
const validation = await validateEvidencePackage({ root, report, reportPath: relative(root, reportPath), requireCompleted: false });
...
console.error(validation.ok ? "CH-001R-r2 aggregate passed." : `CH-001R-r2 aggregate incomplete: ${validation.errors.length} validation issue(s).`);
